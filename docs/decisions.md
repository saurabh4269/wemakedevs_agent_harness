...
12. **Image deploy does not re-import LOOP.** Postgres holds the agent. Prefer porting Render Postgres onto Heroku (`heroku pg:push` or `pg_dump`/`pg_restore`, skip Redis) so agent `loop`, connectors, Daytona, and sessions come along. `import-loop.ts` against the judge URL with Luna env vars is the empty-DB fallback, and still the path after `agents/loop.json` / skills change. Do not store `RENDER_API_KEY` or `HEROKU_API_KEY` in git. Do not restore a dump from a public HTTP URL.
