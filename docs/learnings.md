...
41. **Heroku container TLS + registry.** `node:22-bookworm-slim` needs `ca-certificates` for system CAs (Node still won’t use them without `--use-openssl-ca`). Heroku Postgres TLS verify still needs `NODE_TLS_REJECT_UNAUTHORIZED=0` on the demo host. Heroku’s Docker build context is the Dockerfile directory, so ship a root `Dockerfile` (keep `deploy/trueforge.Dockerfile` in sync for Render). Plain `docker push` to `registry.heroku.com` can return `unsupported`; push with buildx `oci-mediatypes=false`. Laptop `pg_dump` must be Postgres 17 (Docker `postgres:17`); local client 14 aborts. Empty Render `ipAllowList` blocks external dump — temporarily allow `0.0.0.0/0` for the port.

42. **Drop `heroku-redis:mini` — colocate Redis.** TrueForge `STANDALONE=false` still dials Redis (defaults to `127.0.0.1:6379`). Destroying the mini addon without a sidecar crashes the dyno. Install `redis-server` in the image and start it from `start-hosted-trueforge.ts`. Bill becomes Basic ~$7 + essential-0 ~$5 ≈ **$12/mo** (Education credit). Keep Basic, not Eco.
