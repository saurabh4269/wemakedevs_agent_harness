...
- Wake: `GET /healthz` → `OK!`.
- Image deploy does not re-import LOOP. Heroku cutover: **port Render Postgres first** ([heroku.md](heroku.md) §6); skip Redis. After `agents/loop.json` or skills change (or if dump/push failed), import against the judge URL with `LOOP_MODEL_FQN=openai/gpt-5-6-luna` and `OPENAI_MODEL_ID` / `OPENAI_MODEL_NAME`. `import-loop.ts` refuses a judge-host import that is not that FQN.
