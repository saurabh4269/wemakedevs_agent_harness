...
Rewrite this file when PRs, blockers, or URLs change. Do not append a log. Last rewrite: **2026-08-31 ~02:10 IST**.
...
| [#2](https://github.com/saurabh4269/wemakedevs_agent_harness/pull/2) | `feat/loop-ui` | LOOP TrueForgeUI shell for the write-pause demo | Open. Head SHA 8c96416: workspaces plus no client token. Qodo follow-up in flight. Do not clobber. |
| [#3](https://github.com/saurabh4269/wemakedevs_agent_harness/pull/3) | `docs/agent-handoff` | AGENTS.md rewrite + `docs/` | This docs PR. Open against `main` (retargeted from `feat/loop-trueforge-agent`; rebased onto `f1651fa`). Qodo follow-up: strip personal email; rewrite shared-tool-set rules honestly. |
