...
Rewrite this file when PRs, blockers, or URLs change. Do not append a log. Last rewrite: **2026-08-31 ~21:20 UTC**.
...
| [#15](https://github.com/saurabh4269/wemakedevs_agent_harness/pull/15) | `cursor/heroku-standup-7d0f` | Heroku stand-up docs + root Dockerfile TLS | **Merged** (`ddc9651`). |
| [#16](https://github.com/saurabh4269/wemakedevs_agent_harness/pull/16) | `cursor/heroku-cost-colocate-redis-7d0f` | Colocate Redis in the image; drop mini addon | **Merged** (`7e81ebe`). |
| [#17](https://github.com/saurabh4269/wemakedevs_agent_harness/pull/17) | `cursor/heroku-gha-deploy-7d0f` | GitHub Actions deploy to Heroku on `main` | **Merged** (`edc9db8`). |

`origin/main` tip at last rewrite: `edc9db8`. Heroku app **`loop-trueforge`** is live. Render Postgres was **ported**. Peering Redis is **colocated** in the web image (no `heroku-redis:mini`). Bill ≈ Basic $7 + essential-0 $5 ≈ **$12/mo**. Do not store `HEROKU_API_KEY` or dumps in git.

## Live TrueForge

### Judge host

- **Judge URL:** https://loop.heisenbug.in — keep this. `GET /healthz` → 200 `OK!` (Heroku ACM issued). Confirmed 2026-08-31 ~21:19 UTC (`Server: Heroku`). Fallback https://loop-trueforge-17da5f0d6aa2.herokuapp.com/healthz also `OK!`.
...
- **Render Postgres was ported** into Heroku `DATABASE_URL`. Confirmed: `GET /api/v1/agents` returns agent `loop` (`01m1aaemb86czjax2v232nxygf`, Luna). Film session `01m1b50dbbh3vgy6brbaw5vsaz` is on this Postgres. Do not Approve/Deny it.
...
- `NODE_TLS_REJECT_UNAUTHORIZED=0` remains for Heroku Postgres TLS verify (Node does not use system CAs by default).
