...
- **Judge host:** https://loop.heisenbug.in is **live on Heroku** app `loop-trueforge` (`/healthz` → `OK!`). Runbook: [docs/heroku.md](docs/heroku.md). Do **not** Apply `render.yaml`. Do not put `HEROKU_API_KEY`, `RENDER_API_KEY`, or dumps in git.
